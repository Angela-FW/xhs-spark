...
- **点「生成」**：才弹出登录 / 注册（保护 AI 额度，并开启云端同步）。
- **生图 Key**：注册时可展开配置，也可跳过；之后在生成页随时补配（存在本机浏览器）。
...
1. **Supabase（免费项目）**
   ...
   - SQL Editor 执行仓库里的 [`supabase/schema.sql`](supabase/schema.sql)
...
3. **生图 Key（用户自备，异步配置）**
   ...
   - 注册弹窗或生成页填写 Account ID + API Token；不配也能先写文案
   - 服务端也可在 `.env.local` 配 `CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_API_TOKEN` 作兜底（文案生成接口会用到）

登录后规划数据会同步到 Supabase；清浏览器缓存也可找回。仍建议偶尔「导出备份」。
